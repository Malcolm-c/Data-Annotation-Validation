...
function render_original() {
    ...
    data.forEach(function (e) {
        paragraph.innerHTML += `
            <div id="s${e.id}" class="sentence_no_choose" onclick="update_select('${e.id}', 'pos')" >
                ${e.word}
            </div>`
    })
...
function highlight_word(){
    ...
    data.forEach(function (e) {
        console.log(e.id);
        if(e.id % 12 == 1 && e.stop == 0){
            document.getElementById(`s${e.id}`).style.background = "#a9a7ff";
        }
    })
}
...
function show_qa() {
    ...
    document.getElementById("tr_0").style.backgroundColor = "#a9a7ff";
    ...
    CreateQAInst.innerHTML = `<p><b>Your co-worker chose another triple!</b></p>`;
    CreateQAInst.innerHTML += `<table><tr id="tr_0!" onclick="get_row('tr_0')"><td width="30px"><input type="radio" id="radio_0" name="TripleSelected" value="tr_0"></td><td width="100px"><p1 style="background-color: #FFA5A5">die</p1></td><td width="300px">has subevent</td><td width="300px">stop breathing</td></tr></table>`
...
function sub() {
    ...
    if (sub_time == 0){
        ...
        CreateQAInst.innerHTML = `<p><b>Your co-worker wrote the <p7>question</p7> below about this triple.</b></p><p>Now please answer the <b><p7>question</p7></b> based on <p1>the word</p1> <p2>"${marked_word}"</p2>.</p> 
        <p6>· Preferrably including <p2>"${marked_word}"</p2> and <p4>related concept</p4> in your answer.</p6><br>
        <p6>· You can use its <p3>meaning in Wiktionary</p3>.</p6><br>
        <p6>· The QA-pair should be beneficial for children's education.</p6>`
        document.getElementById("question").value = "What happens after someone dies?";
